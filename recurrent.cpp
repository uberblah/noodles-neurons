...
#define ERROR 0.001f
#define ACTIVATION FANN_SIGMOID_SYMMETRIC
...
int main(int argc, char* argv[]) {
    ...
    //load neural network or make a new one
    if(lflag) {
        nNetwork = fann_create_from_file(loadFileName);
    }
    else {
        nNetwork = fann_create_standard(NUM_LAYERS, NUM_IN_OUT * 2, NUM_IN_OUT, NUM_IN_OUT * 2);
	fann_set_activation_function_hidden(nNetwork, ACTIVATION);
	fann_set_activation_function_output(nNetwork, ACTIVATION);
    }

    //train from file if requested
    if(tflag) {
	//open the input file
	FILE* tfile = fopen(trainFileName, "r");
	//set up buffers for input and output
	fann_type* input = (fann_type*)malloc(sizeof(fann_type) * NUM_IN_OUT * 2);
	fann_type* memoin = input + NUM_IN_OUT;
	//load zeroes into the initial memo-in buffer
	for(int i = 0; i < NUM_IN_OUT; i++) memoin[i] = 0.0f;
	fann_type* output = (fann_type*)malloc(sizeof(fann_type) * NUM_IN_OUT * 2);
	fann_type* memoout = input + NUM_IN_OUT;

	//read through any initial zeroes
	...
	//load initial input and output buffers
